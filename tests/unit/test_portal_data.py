...
def test_parse_brl_currency_returns_none_for_empty_string() -> None:
    assert PortalDataFormatter.parse_brl_currency("") is None
    assert PortalDataFormatter.parse_brl_currency("   ") is None
...
def test_parse_brl_currency_converts_brl_string_to_decimal() -> None:
    assert PortalDataFormatter.parse_brl_currency("R$ 1.234,56") == Decimal("1234.56")


def test_format_brl_currency() -> None:
    assert PortalDataFormatter.format_brl_currency(Decimal("1234.56")) == "R$ 1.234,56"
    assert PortalDataFormatter.format_brl_currency(Decimal("0")) == "R$ 0,00"
    assert PortalDataFormatter.format_brl_currency(None) == "R$ 0,00"
