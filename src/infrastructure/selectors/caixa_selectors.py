...
class Selectors(StrEnum):
    ...
    PASSWORD_INVALID_ALERT = (
        "//section[@id='template-section']//div[contains(@class,'alert-error') and normalize-space()='Senha inválida.']"
    )
    ...
    DO_NOT_SHOW_NOTIFICATION_CHECKBOX = (
        "//div[contains(@class,'modal-notificacao')]//input[@type='checkbox' and contains(@aria-label,'mostrar mais')]"
    )
    CLOSE_NOTIFICATION_BUTTON = (
        "//div[contains(@class,'modal-notificacao')]//button[contains(@class,'btn-fechar-notificacao')]"
    )
    ...
    ACCEPT_TERMS_OF_USE_BUTTON = (
        "//button[contains(@class,'data-finalizarcadastro') and normalize-space()='Aceitar Termos de Uso']"
    )
    ...
    CLOSE_BET_REGISTRATION_ALERT_BUTTON = (
        "//div[contains(@class,'modal-content')]"
        "[.//p[contains(., 'O registro de apostas individuais para o concurso') and contains(., 'foi encerrado')]]"
        "//button[@id='fecharModalAlerta']"
    )
    # Realiza aposta
    ...
    # Confirmar pagamento
    CONFIRM_MODAL_NO_REVIEW_CART_BUTTON = (
        "//div[@id='simnao-cancel']//button[contains(@class,'data-cancelar-modal-confirmacao')]"
    )
    CONFIRM_PURCHASE_BUTTON = "//div[@id='ModalConfirmacaoValor']//button[@id='confirma']"
    DAILY_PURCHASE_LIMIT_ALERT_CLOSE_BUTTON = (
    ...
    # BET_CANNOT_BE_PROCESSED_ALERT:
    # "//div[@id='div_alertas' and contains(normalize-space(.), 'não poderão ser efetivadas')]"
    # Mensagem esperada: concurso/modalidade indisponível para captação.
    # OVERNIGHT_BET_PROCESSING_ALERT:
    # "//div[@id='div_alertas' and contains(normalize-space(.), 'apostas realizadas entre 22:00:00 e 05:00:00')]"
    # Mensagem esperada: pagamento recebido aguardando efetivação a partir das 05:00.
    ...
    def mercado_pago_card_icon(credit_card_last_digits: str) -> str:
        return (
            "//div[@id='mercadoPago']//span[contains(@class,'icon-cartao') "
            "and contains(@class,'glyphicon-chevron-right')]/ancestor::tr"
            f"//preceding-sibling::td//h4[contains(normalize-space(.), '{credit_card_last_digits}')]"
        )
    ...
    def purchase_details_value(purchase_details: str) -> str:
        return (
            "//div[contains(@class,'cabecalho_compra_detalhada')]"
            f"//div[contains(normalize-space(.), '{purchase_details}')]/span/b"
        )
    ...
    def purchase_totals_value(purchase_totals: str) -> str:
        return (
            "//div[contains(@class,'totais_compra_detalhada')]"
            f"//div[contains(normalize-space(.), '{purchase_totals}')]"
            "/ancestor::div[contains(@class,'float-left')]"
            "/following-sibling::div[contains(@class,'float-right')][1]//strong"
        )
