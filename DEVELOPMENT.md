...
`BROWSER_PROFILE_DIR` define o perfil persistente do Chromium e é criado automaticamente pelo adapter Playwright. `BROWSER_HEADLESS` aceita `true/false`, `yes/no`, `sim/não` e `1/0`. `BROWSER_TIMEOUT_SECONDS` precisa ser maior que zero e é aplicado como timeout padrão do contexto Playwright e nas navegações.
...
- Manter `CONFIRM_PAYMENT=false` por padrão.
